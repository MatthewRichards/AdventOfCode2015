...
namespace AdventOfCode2015
{
  class Day6
  {
    public void Run()
    {
      var instructions = Input.SplitLines().Select(line =>
      {
        ...
        return Tuple.Create(x1, y1, x2, y2,
          command == "turn on" ? Commands.TurnOn :
          (command == "turn off" ? Commands.TurnOff : Commands.Toggle));
      }).ToList();
      ...
      for (int x = 0; x < 1000; x++)
      {
        for (int y = 0; y < 1000; y++)
        {
          ...
          {
            if (instruction.Item1 <= x && instruction.Item3 >= x &&
              instruction.Item2 <= y && instruction.Item4 >= y)
            {
      ...
      Console.WriteLine($"Brightness: {lightsOn}");
    }
    ...
    private static readonly Regex parser =
      new Regex(@"(?<command>(turn off|turn on|toggle)) (?<x1>\d+),(?<y1>\d+) through (?<x2>\d+),(?<y2>\d+)");
